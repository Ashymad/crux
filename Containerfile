...
FROM ghcr.io/ashymad/crux:${CRUX_VERSION}-stage1 AS build
...
FROM build AS build:attr
RUN cd /usr/ports/core/attr && pkgmk -d && mv -v "attr#2.6.0-1.pkg.tar.gz" $CRUX_REPO
...
RUN cd /usr/ports/core/jansson && pkgmk -d && mv -v "jansson#2.15.1-1.pkg.tar.gz" $CRUX_REPO
...
RUN cd /usr/ports/core/nasm && pkgmk -d && mv -v "nasm#3.02-1.pkg.tar.gz" $CRUX_REPO
...
RUN cd /usr/ports/core/tar && pkgmk -d && mv -v "tar#1.35-2.pkg.tar.gz" $CRUX_REPO
...
FROM build AS build:acl
COPY --from=build:attr $CRUX_REPO/ $CRUX_REPO/
ADD prtlbx pkgdad /usr/bin/
RUN pkgdad -a $CRUX_REPO
RUN cd /usr/ports/core/acl && pkgmk -d && mv -v "acl#2.4.0-1.pkg.tar.gz" $CRUX_REPO
...
COPY --from=build:acl $CRUX_REPO/ $CRUX_REPO/
COPY --from=build:ncurses $CRUX_REPO/ $CRUX_REPO/
ADD prtlbx pkgdad /usr/bin/
RUN pkgdad -a $CRUX_REPO
...
ADD prtlbx pkgdad /usr/bin/
RUN pkgdad -a $CRUX_REPO
RUN cd /usr/ports/core/openssh && pkgmk -d && mv -v "openssh#10.4p1-1.pkg.tar.gz" $CRUX_REPO
...
ADD prtlbx pkgdad /usr/bin/
RUN pkgdad -a $CRUX_REPO
RUN cd /usr/ports/core/python3-setuptools && pkgmk -d && mv -v "python3-setuptools#83.0.0-1.pkg.tar.gz" $CRUX_REPO
...
COPY --from=build:tar $CRUX_REPO/ $CRUX_REPO/
